...
use ratatui::layout::{Rect, Position, Layout, Direction, Constraint};
use vt100::{MouseProtocolMode, MouseProtocolEncoding};
...
pub fn handle_click(app: &mut App, mouse: MouseEvent, size: Rect, tx: &mpsc::Sender<Message>) {
    ...
    if app.show_settings_modal {
        let area = modals::centered_rect_fixed(50, 20, size);
        if area.contains(Position::new(mouse.column, mouse.row)) {
            let block = ratatui::widgets::Block::default().borders(ratatui::widgets::Borders::ALL);
    ...
    if layout.term_area.contains(Position::new(mouse.column, mouse.row)) && mouse_mode != MouseProtocolMode::None {
    ...
    if mouse.kind == MouseEventKind::Down(MouseButton::Left) {
        if layout.scrollbar_area.contains(Position::new(mouse.column, mouse.row)) {
    ...
    let tabs_area = Rect::new(layout.sidebar_area.x, layout.sidebar_area.y, layout.sidebar_area.width, 1);
    let (new_button_area, search_area, list_area) = if app.sidebar_mode == SidebarMode::Gists && !is_sidebar_locked {
        (
            Some(Rect::new(layout.sidebar_area.x, layout.sidebar_area.y + layout.sidebar_area.height.saturating_sub(1), layout.sidebar_area.width, 1)),
            Rect::new(layout.sidebar_area.x, layout.sidebar_area.y + 1, layout.sidebar_area.width, 1),
            Rect::new(layout.sidebar_area.x, layout.sidebar_area.y + 2, layout.sidebar_area.width, layout.sidebar_area.height.saturating_sub(3))
        )
    } else {
        (
            None,
            Rect::new(layout.sidebar_area.x, layout.sidebar_area.y + 1, layout.sidebar_area.width, 1),
            Rect::new(layout.sidebar_area.x, layout.sidebar_area.y + 2, layout.sidebar_area.width, layout.sidebar_area.height.saturating_sub(2))
        )
    };
    ...
    if list_area.contains(Position::new(mouse.column, mouse.row)) && !is_app_active {
                ...
                if index < filtered.len() {
                    ...
                    if is_double_click {
                        ...
                        let is_script = if app.sidebar_mode == SidebarMode::Gists {
                            app.gist_infos.get(original_index).map(|inf| inf.starts_with("__SCRIPT__")).unwrap_or(false)
                        } else {
                            app.sidebar_infos.get(original_index).map(|inf| inf.starts_with("__SCRIPT__")).unwrap_or(false)
                        };
    ...
    } else if layout.sidebar_scrollbar_area.contains(Position::new(mouse.column, mouse.row)) || app.is_dragging_sidebar_scrollbar {
        ...
        match mouse.kind {
            ...
            MouseEventKind::Drag(MouseButton::Left) | MouseEventKind::Moved if app.is_dragging_sidebar_scrollbar => {
                let track_height = layout.sidebar_scrollbar_area.height as f32;
                let mouse_y = mouse.row.clamp(layout.sidebar_scrollbar_area.y, layout.sidebar_scrollbar_area.bottom().saturating_sub(1));
    ...
    } else if layout.scrollbar_area.contains(Position::new(mouse.column, mouse.row)) || app.is_dragging_scrollbar {
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
            ...
            MouseEventKind::Drag(MouseButton::Left) | MouseEventKind::Moved if app.is_dragging_scrollbar => {
                ...
                let screen = parser.screen_mut();
                let track_height = layout.scrollbar_area.height.saturating_sub(2) as f32;
                let mouse_y = mouse.row.clamp(layout.scrollbar_area.y + 1, layout.scrollbar_area.bottom().saturating_sub(2));
    ...
    } else if layout.term_area.contains(Position::new(mouse.column, mouse.row)) || app.is_selecting {

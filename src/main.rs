...
use vt100::Parser;
use std::sync::mpsc;
use crate::app::{App, Message, load_commands};
...
fn main() -> Result<(), Box<dyn Error>> {
            ...
            for (i, label) in labels.iter().enumerate() {
                if label == search_label && sidebar_infos[i].starts_with("__SCRIPT__") {
                    ...
                    let gist_info = if let Some(mtime) = sidebar_mtimes[i] {
                        format!(" [gist from local cache, saved {} ago]", utils::format_time_passed(mtime))
                    } else {
                        String::new()
                    };
    ...
    let cols = if size.width < 106 { size.width } else { size.width.saturating_sub(25) };
                ...
                if let Some(path) = app.update_stats(&mut sys) {
                    if let Some(token) = &app.auth_token {
                        ...
                        if let Some(remote_name) = remote_name {
                            ...
                            std::thread::spawn(move || {
                                let _ = tx.send(Message::GistUploadStatus(format!("[Auto-Uploading Gist: {}]", display_name), true));
                                if let Err(e) = github::upload_gist(&token, &path, &remote_name) {
                                    let _ = tx.send(Message::GistUploadStatus(format!("✗ Auto-upload failed: {}", e), false));
                                } else {
                                    let _ = tx.send(Message::GistUploadStatus(format!("✓ Auto-upload success: {}", display_name), true));
                                }
                            });
                        }
                    }
            ...
            Message::DeleteGist(idx) => {
                if idx < app.gist_paths.len() {
                    ...
                    if let Some(token) = token_opt {
                        std::thread::spawn(move || {
                            let _ = tx.send(Message::GistUploadStatus(format!("[Deleting Gist: {}]", remote_name), true));
                            match crate::github::delete_gist(&token, &remote_name) {
                                Ok(_) => {
                                    let _ = tx.send(Message::GistUploadStatus(format!("✓ Gist deleted: {}", remote_name), true));
                                }
                                Err(e) => {
                                    let _ = tx.send(Message::GistUploadStatus(format!("✗ Gist deletion failed: {}", e), false));
                                }
                            }
                        });
                    }
                ...
                if let Ok(_) = std::fs::write(&path, content) {
                    if let Ok(metadata) = std::fs::metadata(&path) {
                        if let Ok(mtime) = metadata.modified() {
                            ...
                            app.gist_commands.insert(0, path.to_string_lossy().to_string());

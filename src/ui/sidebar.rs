...
pub fn render_sidebar(f: &mut Frame, app: &mut App, area: Rect, scrollbar_area: Rect) {
    ...
    if is_sidebar_locked {
        ...
        let tabs_bg = Color::Black;
        f.render_widget(
            Block::default().style(Style::default().bg(tabs_bg)),
            tabs_area,
        );
        ...
        for (i, (mode, label, color)) in modes.iter().enumerate() {
            ...
            let style = if is_active {
                ...
                Style::default().fg(*color).bg(tabs_bg)
            };
    ...
    let cursor_char = if app.is_search_focused && (now / 500) % 2 == 0 {
        "_"
    } else {
        " "
    };
    ...
    let sidebar_list_items: Vec<ListItem> = filtered_items
        .iter()
        .enumerate()
        .map(|(idx, (i, item))| {
            let color = match app.sidebar_mode {
                ...
                SidebarMode::Commands => {
                    if commands[*i].contains("<prompt") {
                        Color::Green
                    } else if commands[*i].contains("sudo")
                        || (infos.is_some() && infos.unwrap()[*i].to_lowercase().contains("sudo"))
                    {
                        Color::Red
                    } else {
                        Color::White
            ...
            let is_row_hovered = if let Some((mx, my)) = app.mouse_pos {
                list_area.contains(Position::new(mx, my))
                    ...
                    && idx >= offset
                    && idx < offset + list_area.height as usize
            } else {
                false
            };

            let is_button_hovered = is_row_hovered
                && if let Some((mx, _)) = app.mouse_pos {
                    mx == list_area.x + 1
                } else {
                    false
                };

            let is_delete_hovered = is_row_hovered
                && if let Some((mx, _)) = app.mouse_pos {
                    mx >= list_area.x + list_area.width.saturating_sub(4)
                        && mx < list_area.x + list_area.width.saturating_sub(1)
                } else {
                    false
                };
            ...
            let label_text =
                if app.sidebar_mode == SidebarMode::Gists && app.loading_gist && idx == 0 {
                    let now = Instant::now().duration_since(app.start_time).as_millis();
    ...
    let mut sidebar_scrollbar_state =
        ScrollbarState::new(filtered_items.len()).position(sidebar_scroll_pos);
    ...
    f.render_stateful_widget(
        Scrollbar::new(ScrollbarOrientation::VerticalRight)
            ...
            .thumb_style(
                if is_sidebar_scrollbar_hovered || app.is_dragging_sidebar_scrollbar {

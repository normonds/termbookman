use crate::app::App;
use crate::config::{ItemType, StatusBarItem};
...
pub fn render_statusbars(f: &mut Frame, app: &mut App, chunks: &[Rect]) {
    ...
    let mut build_and_render_bar = |bar_items: &Vec<StatusBarItem>,
                                    bar_chunk: Rect,
                                    bar_bg: Color| {
                ...
                ItemType::Button => {
                    if let Some(label) = &item.label {
                        ...
                        let bg_color_str = if is_hovered {
                            item.hover_color
                                .as_deref()
                                .unwrap_or(item.color.as_deref().unwrap_or("white"))
                        } else {
                            item.color.as_deref().unwrap_or("white")
                        };

                        let bg_color = parse_color(bg_color_str);
                        let fg_color = if bg_color_str == "green"
                            || bg_color_str == "cyan"
                            || bg_color_str.starts_with("light_")
                        {
                            Color::Black
                        } else {
                            Color::White
                        ...
                        f.render_widget(
                            Paragraph::new(span).style(Style::default().bg(bar_bg)),
                            chunk,
                        );
                    }
                }
                ...
                ItemType::SelectedCommandInfo => {
                    if let Some(idx) = app.sidebar_state.selected() {
                        let (items, infos, commands) = match app.sidebar_mode {
                            crate::app::SidebarMode::Commands => (
                                &app.sidebar_items,
                                Some(&app.sidebar_infos),
                                &app.sidebar_commands,
                            ),
                            crate::app::SidebarMode::History => {
                                (&app.history_items, None, &app.history_commands)
                            }
                            crate::app::SidebarMode::Gists => {
                                (&app.gist_items, Some(&app.gist_infos), &app.gist_commands)
                            }
                        };
                        ...
                        if idx < filtered.len() {
                            ...
                            let info = if let Some(infos) = infos {
                                &infos[original_index]
                            } else {
                                ""
                            };

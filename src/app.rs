use crate::config::{load_config, ConditionType, Config, StatusBarItem};
use crate::utils::{log_debug, parse_lines, parse_script_content};
...
pub enum Message {
    ...
    ), // (label, info, command, mtime, path, remote_name)
...
fn check_update_disabled() -> Option<String> {
    // List of privileged directories where auto-updates might cause permission issues
    let privileged_dirs = [
        "/bin",
        "/usr/bin",
        "/usr/local/bin",
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            ...
            // Check if executable is in a privileged directory
            let in_privileged_dir = privileged_dirs
                .iter()
                .any(|&dir| exe_dir_str.starts_with(dir));

            if in_privileged_dir {
                // Check if running with sudo privileges
                let has_sudo = std::env::var("SUDO_USER").is_ok() || {
                    #[cfg(unix)]
                    {
                ...
                // If in privileged dir but NOT running as sudo, disable updates
                if !has_sudo {
                    return Some(
                        "⚠ App in restricted directory. Run with 'sudo' to enable updates."
                            .to_string(),
                    );
                }
            }
...
impl App {
    pub fn is_item_visible(&self, item: &StatusBarItem) -> bool {
        match &item.condition {
            ...
            Some(ConditionType::HasSelection) => {
                self.selection_start.is_some() && self.selection_end.is_some() && !self.is_selecting
            }
            None => true,
        }
    }
    ...
    pub fn process_prompt(&self, cmd: &mut String) {
        while let Some(start) = cmd.find("<prompt") {
            if let Some(end) = cmd[start..].find(">") {
                ...
                let prompt_name = prompt_str.trim_matches(|c| {
                    c == '<'
                        || c == '>'
                        || c == 'p'
                        || c == 'r'
                ...
                let input = if let Ok(output) = std::process::Command::new("bash")
                    .arg("-c")
                    .arg(format!("read -p '{}: ' val && echo $val", prompt_name))
                    .output()
                {
    ...
    pub fn new(
        ...
        let auth_token = config
            ...
            .clone()
            .filter(|t| !t.is_empty() && t != "YOUR_TOKEN_HERE");
        App {
            cpu_usage: 0.0,
            mem_usage: (0, 0),
            git_info: None,
            ...
            update_url_input: {
                let mut url = config.auth.update_url.clone();
                let is_arm = std::env::consts::ARCH == "aarch64";
                if is_arm {
                    url = url.replace("/download/tbm", "/download/tbm.arm");
                }
                url
            },
            is_update_url_focused: false,
    ...
    pub fn copy_selection(&mut self) {
        ...
        if let (Some(start), Some(end)) = (self.selection_start, self.selection_end) {
            ...
            for r in min_row..=max_row {
                ...
                let end_c = if r == max_row {
                    max_col
                } else {
                    (cols as u16).saturating_sub(1)
                };

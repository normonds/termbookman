...
impl<CB: crate::callbacks::Callbacks> vte::Perform for WrappedScreen<CB> {
    fn print(&mut self, mut c: char) {
        let is_acs = if self.shift_out { self.g1_is_acs } else { self.g0_is_acs };
        if is_acs && (c >= '\x60' && c <= '\x7e') {
            c = match c {
                '`' => '◆', 'a' => '▒', 'b' => '␉', 'c' => '␌',
                'd' => '␍', 'e' => '␊', 'f' => '°', 'g' => '±',
                'h' => '␤', 'i' => '␋', 'j' => '┘', 'k' => '┐',
    ...
    fn esc_dispatch(&mut self, intermediates: &[u8], _ignore: bool, b: u8) {
        if let Some(i) = intermediates.first() {
            if *i == b'(' {
                if b == b'0' { self.g0_is_acs = true; return; }
                if b == b'B' { self.g0_is_acs = false; return; }
            } else if *i == b')' {
                if b == b'0' { self.g1_is_acs = true; return; }
                if b == b'B' { self.g1_is_acs = false; return; }
            }
